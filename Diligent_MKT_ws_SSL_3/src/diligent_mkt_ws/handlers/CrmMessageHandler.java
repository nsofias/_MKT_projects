...
public class CrmMessageHandler implements SOAPHandler<SOAPMessageContext>
    ...
    public boolean handleMessage(SOAPMessageContext messageContext)
        ...
        if (outInd)
            ...
            try
                {
                outlogOUT = new FileOutputStream(new File(System.getenv("APPLICATIONS_PATH") + "/xml/" + new TimeStamp1().getNowUnformated() + "MARK_CUSTOMERS_TO_CRM_OUT.xml"), true);
                if (MODE == 1)
                    {
        ...
        //-------------------------
            ...
            try
                {
                outlogIN = new FileOutputStream(new File(System.getenv("APPLICATIONS_PATH") + "/xml/" + new TimeStamp1().getNowUnformated() + "MARK_CUSTOMERS_TO_CRM_IN.xml"), true);
                if (MODE == 1)
                    {
