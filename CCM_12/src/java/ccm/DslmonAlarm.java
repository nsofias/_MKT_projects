...
public class DslmonAlarm implements Comparable
    ...
    public boolean updateAlarm(long numberOfActiveUsers,String now)
        {
        boolean closed=false;
        TimeStamp1 T2 = new TimeStamp1(getAlertStop());                
        if (this.isStilAlive()) 
            {
            TimeStamp1 T1 = new TimeStamp1(getAlertStart());
            double duration = T2.hoursDiff(T1);                
            if (numberOfActiveUsers>0 || duration>24)
                {
                setStilAlive(false);
                closed=true;
                }
